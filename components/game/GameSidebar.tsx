...
export const GameSidebar: React.FC<GameSidebarProps> = ({ roomData, currentUser }) => {
  ...
  return (
    <div className="w-80 bg-luxury-dark border-l border-gold-900 flex flex-col hidden xl:flex shadow-[-10px_0_20px_rgba(0,0,0,0.5)] z-30 h-full">
          
          {/* Player List Section (Top 60%) */}
          <div className="flex-[3] flex flex-col min-h-0 border-b-4 border-gold-900/20">
              <div className="p-3 border-b border-gold-800/30 bg-black/40 flex items-center justify-between shrink-0">
              ...
              <div className="flex-1 p-3 space-y-2 overflow-y-auto bg-black/20 custom-scrollbar">
                {roomData.playerOrder.map(uid => {
                  ...
                  return (
                  <div key={player.id} className={`p-3 rounded-sm border relative transition-all 
                  ...
                  `}>
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-gray-700 to-black border border-gray-600 flex items-center justify-center">
                          ...
                          <div>
                            <div className="text-xs font-bold text-gray-200 flex items-center gap-2">
                                {player.name}
                                {player.id === currentUser?.uid && <span className="text-[9px] text-gold-500 bg-gold-900/30 px-1 rounded">YOU</span>}
                            </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div className="bg-black/40 p-1.5 rounded border border-white/5">
                          <div className="text-gray-500 mb-0.5 flex items-center gap-1"><Coins size={8}/> BALANCE</div>
                          <div className="font-mono text-gray-300">₩ {formatPrice(player.balance).replace('억', '')}</div>
                        </div>
                        <div className="bg-black/40 p-1.5 rounded border border-white/5">
                          <div className="text-gray-500 mb-0.5 flex items-center gap-1"><Building2 size={8}/> ASSETS</div>
          ...
          <div className="flex-[2] min-h-0 bg-[#0a0a0a]">
             <GameChat 
                roomId={roomData.id} 
